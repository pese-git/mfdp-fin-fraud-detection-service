...
from src.services.logging.logging import get_logger  # Импортируем наш логгер
...
class AccessControl:
    ...
    def can_access(self, role: str, resource: str, action: str) -> bool:
        ...
        if policy:
            ...
            logger.debug(
                "Доступ РАЗРЕШЁН: роль='%s', ресурс='%s', действие='%s'", role, resource, action
            )
        else:
            # Логируем отказ в доступе
            logger.warning(
                "Доступ ЗАПРЕЩЁН: роль='%s', ресурс='%s', действие='%s'", role, resource, action
            )
