...
def _transaction_df_to_json(df: pd.DataFrame) -> List[PredictionCreate]:
    """
    Преобразует DataFrame обратно в list[PredictionCreate] с вложенными массивами C, D, M, V, IDs
    ...
    for _, row in df.iterrows():
        ...
        records.append(PredictionCreate(**record))
...
class AntifraudPredictor:
    ...
    def predict(self, input_json: Union[Dict, List[Dict]]) -> List[PredictionCreate]:
...
def antifraud_predict(input_json: Union[Dict, List[Dict]]) -> List[PredictionCreate]:
    """
...
def do_task(input_json: Any) -> List[PredictionCreate]:
    """
